...
describe('Stream type auto detection', function() {
  ...
  describe('when recieve ULR', () => {
    ...
    it('should detect type even if it has query params', () => {
      expect(detectStreamType(mp4URL + queryParam)).to.equal(
        MEDIA_STREAM_TYPES.MP4,
      );
    });

    it('should detect type even if it has fragments', () => {
      expect(detectStreamType(mp4URL + fragment)).to.equal(
        MEDIA_STREAM_TYPES.MP4,
      );
    });

    it('should detect type even if it has fragments and params', () => {
      expect(detectStreamType(mp4URL + queryParam + fragment)).to.equal(
        MEDIA_STREAM_TYPES.MP4,
      );
    });
  });
  ...
  it("should throw error if can't parse url", () => {
    expect(() => detectStreamType('test.url')).to.throw(
      "Vidi: cannot auto-detect url 'test.url'. Please specify type manually using the MediaStream interface.",
    );
  });
});
