...
describe('AdapterStrategy', () => {
  ...
  it('should generate list of available stream creator in env on construction', () => {
    const availableStream = getNativeAdapterCreator(MEDIA_STREAM_TYPES.HLS, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_ADAPTIVE);
    const unavailableStream = getNativeAdapterCreator(MEDIA_STREAM_TYPES.DASH, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_ADAPTIVE);
    availableStream.isSupported = () => true;
    unavailableStream.isSupported = () => false;

    const newStrategy: any = new AdapterStrategy(eventEmitter, video, [availableStream, unavailableStream]);
    expect(newStrategy._availableAdapters.length).to.be.equal(1);
    expect(newStrategy._availableAdapters[0].constructor).to.be.equal(availableStream);
  });
  ...
  it('should choose proper media stream for proper format', () => {
    playbackAdapters = [
      ...
      getNativeAdapterCreator(MEDIA_STREAM_TYPES.MP4, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_ADAPTIVE),
    ];
    ...
    expect(strategy.attachedAdapter.mediaStreamType).to.be.equal(MEDIA_STREAM_TYPES.DASH);

    strategy.connectAdapter('http://www.hls.com/hls.m3u8');
    expect(strategy.attachedAdapter.mediaStreamType).to.be.equal(MEDIA_STREAM_TYPES.HLS);

    strategy.connectAdapter({ url: 'http://www.mp4.com/mp4.mp4', type: MEDIA_STREAM_TYPES.MP4 });
  });

  it('should choose proper media stream based on priority', () => {
    playbackAdapters = [
      getNativeAdapterCreator(MEDIA_STREAM_TYPES.DASH, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_PROGRESSIVE),
      getNativeAdapterCreator(MEDIA_STREAM_TYPES.DASH, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_ADAPTIVE),
    ];
    ...
    strategy.connectAdapter('http://www.dash.com/dash.mpd');
    expect(strategy.attachedAdapter.mediaStreamDeliveryType).to.be.equal(MEDIA_STREAM_DELIVERY_TYPE.NATIVE_ADAPTIVE);

    playbackAdapters = [
      getNativeAdapterCreator(MEDIA_STREAM_TYPES.HLS, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_ADAPTIVE),
      getNativeAdapterCreator(MEDIA_STREAM_TYPES.HLS, MEDIA_STREAM_DELIVERY_TYPE.NATIVE_PROGRESSIVE),
    ];
